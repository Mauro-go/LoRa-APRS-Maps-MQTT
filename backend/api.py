...
from config import (
    ...
    DB_FILE,
    MAX_HISTORY_PACKETS,
    RECEIVERS as CONFIG_RECEIVERS,
)

HOST = API_HOST
PORT = API_PORT

# The original API internally uses callsign -> mqtt_name.
# Public configuration uses mqtt_name -> callsign because this matches
# the MQTT topic and is easier to maintain.
RECEIVERS = {
    cfg["callsign"]: {
        "mqtt_name": mqtt_name,
        "enabled": bool(cfg.get("enabled", True)),
    }
    for mqtt_name, cfg in CONFIG_RECEIVERS.items()
    if cfg.get("callsign")
}
...
def parse_aprs_payload(payload, depth=0):
    ...
    if payload.startswith("}"):
        ...
        if colon >= 0:
            ...
            #
            # La posizione appartiene all'oggetto interno
            # (es. radiosonda), NON alla stazione LoRa esterna
            # che ha ritrasmesso il packet.
            #
            # Quindi non dobbiamo usarla come posizione della
            # stazione esterna sulla mappa/traccia storica.
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def api_status(self):
        ...
        for callsign, config in RECEIVERS.items():
            ...
            for beacon_row in beacon_rows:
                parsed_beacon = parse_aprs_packet(
                    beacon_row["packet"]
                )

                if (
                    parsed_beacon.get("latitude") is not None
                    and parsed_beacon.get("longitude") is not None
                ):
                    receiver_position = parsed_beacon
                    break
    ...
    def api_packets(self, parsed):
        ...
        hours_value = params.get("hours", [None])[0]
        ...
        if hours_value is not None:
            ...
            hours = min(max(hours, 0.01), 24 * 31)
        ...
        def get_last_known(callsign):
            ...
            for historical_row in historical_rows:
                ...
                if (
                    ...
                    and parsed_historical.get("longitude") is not None
                ):
