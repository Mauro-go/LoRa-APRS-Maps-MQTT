...
-   accesso a un MQTT broker.

Il MQTT broker può essere:
...
# 4. Package installation necessari
...
## 5.1 Se esiste già un MQTT broker

Se gli iGate pubblicano già verso un MQTT broker funzionante, saltare
...
The repository can be cloned into any directory chosen by the
administrator:
...
# 7.1 File destinations and permissions

La posizione in cui è stato clonato il repository non determina la
posizione finale dei files installati.

Expected destinations:

``` text
backend/
    destination: /opt/lora-aprs/backend/
    files:
        collector.py
        api.py
        config.py
...
web/
    destination: /var/www/html/lora-aprs/
    copiare tutto il contents della directory web/
...
apache/
    contains the reference Apache configuration
```

## Permissions

The procedure described in this guide is performed as `root`.

The two Python backend programs must be executable and must have the
...
therefore with `755` permissions.
...
deve esistere e deve consentire ai servizi di creare e modificare i
files runtime del progetto, in particolare:
...
i servizi systemd forniti dal progetto, directory e files runtime
possono essere mantenuti di proprietà `root:root`.

**IMPORTANT:** permessi errati su `collector.py`, `api.py` oppure sulla
...
Il backend utilizza un unico files locale:
...
-   `MQTT_HOST` e `MQTT_PORT`: MQTT broker;
-   `MQTT_USER` e `MQTT_PASSWORD`: credentials, if required;
-   **`MQTT_TOPIC = "lora_aprs/+/#"`: DO NOT CHANGE.** Questo valore
...
-   `API_HOST` e `API_PORT`: API listening address and port;
-   `RX1`, `RX2`, `RX3`...: internal program identifiers;
-   `callsign`: full APRS callsign of the receiver;
-   `enabled`: enables or disables the receiver.

## 8.1 RX1/RX2 e nome MQTT

`RX1`, `RX2`, `RX3`... **are not the names used in the MQTT topic** and
must not be replaced with them.

For example, if the iGate publishes:
...
The collector recognizes the beacon from the configured callsign,
automatically associates the MQTT name (`IK3ABC` in the example) with
the internal identifier `RX1`, and stores the association in:

``` text
/opt/lora-aprs/data/receiver-map.json
```

The association is reused after subsequent restarts.
...
There is no need to know or enter the MQTT name used by the iGate in
`config.py`: the collector learns it automatically from the beacon.
...
There is no need to create a separate HTML page for each receiver.

------------------------------------------------------------------------

# 10. Protecting the configuration
...
Il files `.gitignore` deve contenere almeno:
...
# 12. Testing manuale del collector
...
# 13. Configuration di rete e test manuale dell'API
...
I files `.service` presenti nella directory `systemd/` del repository
...
Il contents della directory `web/` del repository deve essere installato
nella directory pubblica:
...
The `coverage.html` page is shared by all receivers.

The receiver is selected through a parameter, for example:
...
# 17. Configuration Apache per l'API
...
-   `aprs-symbols.js`;
-   `aprs-symbols.css`;
-   APRS PNG sprites in 24, 48, 64, and 128 pixel sizes.

Tutti i files della libreria APRS devono essere installati in:

``` text
/var/www/html/lora-aprs/aprs-symbols/
```

The project's web pages load APRS symbols and sprites directly from this
directory.
...
Verificare che Apache riesca a servire i files:
...
If a message indicates that an MQTT receiver has not yet been
identified, check that the `callsign` configured for RX1/RX2 exactly
matches the callsign transmitted by the iGate beacon.

È utile verificare anche il files:

``` text
/opt/lora-aprs/data/receiver-map.json
```

which contains the automatically learned MQTT associations.
...
Deve essere sufficiente modificare il files di configurazione.
